...
SenderQueue.prototype.send = function(task, quequeName) {
    ...
    if (task.retry) {
        logger.debug('retrying ' + task.retry + ' ...');
    }
    ...
    let payloadString = (typeof task.payload === 'object') ?
...
SenderQueue.prototype.addTask = function(task) {
    if (task.type === 'dc') {
        if (pluginconfig.dcconsumers.length === 0) {
            logger.info('No consumers on dc queue. give up the payload!');
        ...
        if (pluginconfig.resourceconsumers.length === 0) {
            logger.info('No consumers on resource queue. give up the payload!');
        ...
        if (pluginconfig.metricconsumers.length === 0) {
            logger.info('No consumers on metric queue. give up the payload!');
        ...
        if (pluginconfig.aarconsumers.length === 0) {
            logger.info('No consumers on aar queue. give up the payload!');
        ...
        if (pluginconfig.adrconsumers.length === 0) {
            logger.info('No consumers on adr queue. give up the payload!');
        ...
        if (pluginconfig.jsoconsumers.length === 0) {
            logger.info('No consumers on jso queue. give up the payload!');
