...
function formatMetricPayload(payload) {
    ...
    if (!targetPayload['gc_duration']) {
        targetPayload['gc_heapSize'] = 0;
        targetPayload['gc_heapUsed'] = 0;
        targetPayload['gc_duration'] = 0;
        targetPayload['gc_mCount'] = 0;
        targetPayload['gc_sCount'] = 0;
        targetPayload['gc_iCount'] = 0;
        targetPayload['gc_wCount'] = 0;
    } else {
        if (targetPayload['gc_type'] === 'M') {
            targetPayload['gc_mCount'] += 1;
        } else if (targetPayload['gc_type'] === 'S') {
            targetPayload['gc_sCount'] += 1;
        } else if (targetPayload['gc_type'] === 'W') {
            targetPayload['gc_wCount'] += 1;
        } else {
            targetPayload['gc_iCount'] += 1;
        }
    }
...
function initFromGlobalEnv() {
    let globalenvfile = __dirname + '/../../../../etc/global.environment';
    if (fs.existsSync(globalenvfile)) {
        ...
        if (global_obj.APM_BM_GATEWAY_URL && isURL(global_obj.APM_BM_GATEWAY_URL) &&
            ...
            if (global_obj.APM_KEYFILE) {
                try {
                    let keyfile = __dirname + '/../../../../etc/' + global_obj.APM_KEYFILE;
