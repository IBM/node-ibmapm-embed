    ...
    if (IBAM_options) {
        ...
        if (task.type.indexOf('resources:') === 0) {
            ...
            if (task.payload.entityTypes[0] === 'serviceEndpoint') {
                task.url = wf_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_PATH + '/service_endpoints';
            }
        }
        ...
        if (task.type && task.type.indexOf('amui:') === 0) {
            task.url = wf_url + WF_SVCS_CONN.UI_SERVICE_BASE_PATH + '/bulk';
...
function setServerConnGE() {
    ...
    if (bamurl &&
        ...
        bamconns[bamurl] = {
            ...
            rms_serviceendpoints_url: bamurl + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_PATH + '/service_endpoints',
            ...
            ui_url: bamurl + WF_SVCS_CONN.UI_SERVICE_BASE_PATH + '/bulk',
    ...
    if (pluginConfig.plugins['BAM'] && pluginConfig.plugins['BAM'].connections) {
        ...
        for (var index = 0; index < connections.length; index++) {
            ...
            bamconns[element.server_url] = {
                ...
                rms_resource_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_PATH + '/resources',
                rms_provider_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_PATH + '/providers',
                rms_app_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_PATH + '/services',
                rms_serviceendpoints_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_PATH + '/service_endpoints',
                rms_metadata_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_PATH + '/metadata',
                metric_url: element.server_url + WF_SVCS_CONN.METRICS_SERVICE_BASE_PATH + '/data',
                ui_url: element.server_url + WF_SVCS_CONN.UI_SERVICE_BASE_PATH + '/bulk',
...
function sendMetadata() {
    ...
    task.additionalHeader['X-TenantId'] = tId;
    try {
        var mdsvc = require('../../etc/ResourceTypes/service_1.2_ResourceType.json');
        task['payload'] = JSON.stringify(mdsvc);
        queue.addTask(task);

        var mdruntime = require('../../etc/ResourceTypes/nodeApplicationRuntime_1.2_ResourceType.json');
        task['payload'] = JSON.stringify(mdruntime);
        queue.addTask(task);

        var mdinst = require('../../etc/ResourceTypes/nodeServiceInstance_1.2_ResourceType.json');
        task['payload'] = JSON.stringify(mdinst);
        queue.addTask(task);
    } catch (e) {
        logger.error(e);
    }
