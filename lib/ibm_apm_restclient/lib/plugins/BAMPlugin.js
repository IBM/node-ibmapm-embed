...
var WF_SVCS_CONN = {
    RESOURCEMGMT_SERVICE_BASE_URL: process.env.RESOURCEMGMT_SERVICE_BASE_URL ?
        process.env.RESOURCEMGMT_SERVICE_BASE_URL : pluginConfig.globalEnv.RESOURCEMGMT_SERVICE_BASE_URL || '/applicationmgmt/0.9',
    METRICS_SERVICE_BASE_URL: process.env.METRICS_SERVICE_BASE_URL ?
        process.env.METRICS_SERVICE_BASE_URL : pluginConfig.globalEnv.METRICS_SERVICE_BASE_URL || '/metric/1.0',
    UI_SERVICE_BASE_URL: process.env.UI_SERVICE_BASE_URL ?
        process.env.UI_SERVICE_BASE_URL : pluginConfig.globalEnv.UI_SERVICE_BASE_URL || '/uielement/0.8'
};
...
function sendOne(task, callback) {
    ...
    if (IBAM_options) {
        ...
        if (task.type.indexOf('resources:') === 0) {
            task.url = wf_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/resources';
            if (task.payload.entityTypes[0] === 'application') {
                task.url = wf_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/applications';
            }
            if (task.payload.entityTypes[0] === 'serviceEndpoint') {
                task.url = wf_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/service_endpoints';
            }
        }
        if (task.type && task.type.indexOf('metadata:') === 0) {
            task.url = wf_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/metadata';
        }

        if (task.type && task.type.indexOf('amui:') === 0) {
            task.url = wf_url + WF_SVCS_CONN.UI_SERVICE_BASE_URL + '/bulk';
            if (!(task.payload instanceof Buffer)) {
                task.payload = new Buffer(task.payload);
            }
        }
        if (task.type.indexOf('metrics:') === 0) {
            task.url = wf_url + WF_SVCS_CONN.METRICS_SERVICE_BASE_URL + '/data';
        }
        if (task.type.indexOf('aar:') === 0) {
        ...
        if (task.type.indexOf('dc') === 0) {
            task.url = wf_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/providers';
        }
...
function setServerConnGE() {
    logger.debug('BAMPlugin.js', 'setServerConnGE',
        'Retrive server configuration from global.environment.', process.env.IBM_APM_INGRESS_URL);
    ...
    if (bamurl &&
        ...
        bamconns[bamurl] = {
            server_url: bamurl,
            rms_resource_url: bamurl + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/resources',
            rms_provider_url: bamurl + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/providers',
            rms_app_url: bamurl + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/applications',
            rms_serviceendpoints_url: bamurl + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/service_endpoints',
            rms_metadata_url: bamurl + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/metadata',
            metric_url: bamurl + WF_SVCS_CONN.METRICS_SERVICE_BASE_URL + '/data',
            ui_url: bamurl + WF_SVCS_CONN.UI_SERVICE_BASE_URL + '/bulk',
    ...
    if (pluginConfig.plugins['BAM'] && pluginConfig.plugins['BAM'].connections) {
        ...
        for (var index = 0; index < connections.length; index++) {
            ...
            bamconns[element.server_url] = {
                ...
                rms_resource_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/resources',
                rms_provider_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/providers',
                rms_app_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/applications',
                rms_serviceendpoints_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/service_endpoints',
                rms_metadata_url: element.server_url + WF_SVCS_CONN.RESOURCEMGMT_SERVICE_BASE_URL + '/metadata',
                metric_url: element.server_url + WF_SVCS_CONN.METRICS_SERVICE_BASE_URL + '/data',
                ui_url: element.server_url + WF_SVCS_CONN.UI_SERVICE_BASE_URL + '/bulk',
