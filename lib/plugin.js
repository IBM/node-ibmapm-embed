...
MonitoringPlugin.prototype.init = function(envType) {
    ...
    if (process.env.MONITORING_SERVER_TYPE === 'BAM') {
        // To registry serviceEndpoint and applicationInstance, need to enable 'requests event'.
        healthcenter.enable('requests', requestConfig);
    }
    // for Bluemix & Cloudnative
    if (process.env.APM_BM_SECURE_GATEWAY || process.env.APM_BM_GATEWAY_URL) { // Bluemix
        ...
        if (commonTools.testTrue(process.env.KNJ_ENABLE_METHODTRACE)) {
            ...
            logger.info('plugin.js', 'init', 'enable TT.');
            process.env.KNJ_ENABLE_TT = true;
