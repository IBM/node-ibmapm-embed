...
function Metric(inInterval) {
    ...
    var self = this;
    oninitialized(healthcenter, self);
...
function oninitialized(healthcenter, self) {
...
function onhttps(healthcenter, self) {
    healthcenter.on('https', function(http) {
        if (!global.RESOURCE_SVCEP_REGISTED || global.RESOURCE_SVCEP_REGISTED === 'false') {
            jsonSender.registerAppModelOnPre(http);
        }
        self.addHttpRequest(http, http.duration);
...
Metric.prototype.addHttpRequest = function addHttpRequest(req, responseTime) {
    ...
    this.averageResponseTime = Math.round(this.totalResponseTime / this.totalRequests);
...
Metric.prototype.getGC = function getGC() {
    ...
    if (totalCount) {
        ...
        xmlRow = '  <in>' +
            ...
            '    <a v="' + this.gc.m_count + '"/>' +
            '    <a v="' + this.gc.s_count + '"/>' +
            // TODO: 2018-3-23 need to update Node.js v8 agent xml and dashboard accordingly
            //            '    <a v="' + this.gc.w_count + '"/>' +
            //            '    <a v="' + this.gc.i_count + '"/>' +
            '  </in>';
        xmlString += xmlRow;
    ...
    for (var key in requests) {
        if (requests.hasOwnProperty(key)) {
            ...
            var jsonRequestData = {
                ...
                REQ_RESP_TIME: Math.round(request.averageResponseTime),
