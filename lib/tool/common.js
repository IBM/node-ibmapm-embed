...
exports.envDecrator = function() {
    ...
    if (process.env.IBM_APM_SERVER_URL) {
        ...
        process.env.IBM_APM_SERVER_URL = isbase64 ?
            Buffer.from(process.env.IBM_APM_SERVER_URL, 'base64').toString() :
    ...
    if (process.env.IBM_APM_KEYFILE) {
        process.env.APM_KEYFILE = isbase64 ?
            Buffer.from(process.env.IBM_APM_KEYFILE, 'base64').toString() :
            process.env.IBM_APM_KEYFILE;
    }
    if (process.env.IBM_APM_KEYFILE_URL) {
        process.env.APM_KEYFILE_URL = isbase64 ?
            Buffer.from(process.env.IBM_APM_KEYFILE_URL, 'base64').toString() :
    ...
    if (process.env.IBM_APM_SERVER_INGRESS_URL) {
        ...
        process.env.IBAM_INGRESS_URL = isbase64 ?
            Buffer.from(process.env.IBM_APM_SERVER_INGRESS_URL, 'base64').toString() :
    ...
    if (process.env.IBM_APM_ACCESS_TOKEN) {
        process.env.IBAM_TOKEN = isbase64 ?
            Buffer.from(process.env.IBM_APM_ACCESS_TOKEN, 'base64').toString() :
...
exports.uid_service = function(resourceType, namespace, servicename, svc_uid) {
    return 'NodejsDC_' + namespace + '_' + resourceType + '_' + servicename + '_' + svc_uid;
};

exports.polishServiceName = function(providerId, namespace, servicename) {
    var tid = process.env.APM_TENANT_ID;
    if (tid && providerId && servicename) {
        serviceId = providerId + '_' + namespace + '_k8sService_' + servicename.replace(/\:|\.|\//g, '_');
        return tid + '_' + serviceId;
    } else if (tid && servicename) {
        serviceId = '_default_' + namespace + '_k8sService_' + servicename.replace(/\:|\.|\//g, '_');
        return tid + serviceId;
    } else {
        serviceId = null;
        return servicename;
    }
...
exports.getk8mServiceID = function() {
    return serviceId;
};

exports.uid_onprem_service = function(resourceType, appName, serverUrl, containerId) {
    if (appName.indexOf('/') >= 0 || appName.indexOf(':') >= 0 || appName.indexOf('\\') >= 0) {
        appName = crypto.createHash('md5').update(appName).digest('hex');
    }
    if (containerId) {
        return 'NodejsDC_' + resourceType + '_' + appName + '_' + serverUrl.replace(/\:|\./g, '_')
            + '_' + containerId;
    } else {
        return 'NodejsDC_' + resourceType + '_' + appName + '_' + serverUrl.replace(/\:|\./g, '_');
    }
...
exports.uid_onprem_runtime = function(resourceType, ip, serverDir, appName, containerId) {
    var serverDirMD5 = crypto.createHash('md5').update(serverDir).digest('hex');
    if (appName.indexOf('/') >= 0 || appName.indexOf(':') >= 0 || appName.indexOf('\\') >= 0) {
        appName = crypto.createHash('md5').update(appName).digest('hex');
    }
    if (containerId) {
        return 'NodejsDC_' + resourceType + '_' + ip.replace(/\./g, '_') + '_'
            + serverDirMD5 + '_' + appName + containerId;
    } else {
        return 'NodejsDC_' + resourceType + '_' + ip.replace(/\./g, '_') + '_' + serverDirMD5 + '_' + appName;
    }
};
