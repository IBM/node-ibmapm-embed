...
var k8sutil = require('../ibm_apm_restclient/lib/tools/k8sutil');
...
exports.enableTrace = function(appmetrics) {
    if (this.testTrue(process.env.KNJ_ENABLE_TT) ||
...
exports.envDecrator = function() {
    ...
    logger.debug('The current environments:', process.env);
    if (process.env.APPLICATION_NAME) {
        process.env.MONITORING_APPLICATION_NAME = process.env.APPLICATION_NAME;
    }
    ...
    if (process.env.IBM_APM_SERVER_URL) {
        ...
        process.env.APM_BM_GATEWAY_URL = isbase64 ?
...
exports.getServerPort = function() {
    let handles = global.process._getActiveHandles();
    ...
    handles.forEach(function(handle) {
        if (handle.hasOwnProperty('_connectionKey')) {
            let connkey = handle._connectionKey;
            let conns = connkey.split(':');
