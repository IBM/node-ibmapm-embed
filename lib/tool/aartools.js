...
function processAAROneInteraction(interData) {
    ...
    if (interData.correlator) {
        interaction.interactionCorrelator = interData.type === 'REQUEST' ?
            Buffer.from(interData.correlator).toString('base64') :
            Buffer.from(interData.correlator + 'FF').toString('base64');
    }
    if (interData.stitch) {
...
function processAARInteractions(req, result, port, context) {
    ...
    var stitch_info;
    var correlator;
    if (req.name && req.name.length > global.KNJ_TT_MAX_LENGTH) {
        req.name = req.name.substr(0, global.KNJ_TT_MAX_LENGTH);
    }
    if (!req.type) {
        traceIt = false;
...
exports.composeAARTT = function composeAARTT(data, port) {
    ...
    if (data.name && data.name.length > global.KNJ_TT_MAX_LENGTH) {
        data.name = data.name.substr(0, global.KNJ_TT_MAX_LENGTH);
    }
    var result = {
        metrics: {
    ...
    if (data.request.context && data.request.context.requestHeader
            && data.request.context.requestHeader['X-Synthetic-Token']) {
        result.properties.syntheticTest = true;
    }
    if (statusCode >= 400) {
