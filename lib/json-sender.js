...
JsonSender.prototype.register = function register() { // Register DC and Resouce
    ...
    logger.debug('The plain text of service instance id: ', _this.instanceString);
    ...
    if (this.vcap && !global.RESOURCE_SVCEP_REGISTED) {
        ...
        for (var index = 0; index < this.vcap.application_uris.length; index++) {
            ...
            var interfaceObj = {
                ...
                deployment: getDeployment(),
                properties: {
                    displaylabel: 'Service Endpoints',
                    uri: uri,
                    port: this.vcap.port,
                    name: this.vcap.name,
                    tags: {
                ...
                references: [
                    { direction: 'to', type: 'communicatesWith', id: this.instanceMD5String },
                    { direction: 'from', type: 'monitors', id: this.dcMD5String }
                ]
            };
        ...
        var instanceObj = {
            ...
            references: [
                { direction: 'to', type: 'runsOn', id: this.nodeAppRuntimeMD5String }
            ]

        };
        restClient.registerResource(instanceObj, function(err, res) {
            if (err) { return; }
            if (res && ((res.statusCode >= 200 && res.statusCode < 300) || res.statusCode === 409)) {
                logger.debug('serviceInstance is registered');
                global.serviceInstanceIsRegistered = true;
            }
        });
...
JsonSender.prototype.send = function send(data) {
    ...
    if (global.serviceInstanceIsRegistered && global.nodeApplicationRuntimeIsRegistered &&
