...
JsonSender.prototype.register = function register() { // Register DC and Resouce
    ...
    if (process.env.VCAP_APPLICATION) {
        ...
        _this.nodeRuntimeString = _this.vcap.application_id + 'noderuntime';
    } else if (_this.isicp) {
        ...
        _this.nodeRuntimeString = _this.nodeAppString + 'noderuntime';
    ...
    _this.dcMD5String = crypto.createHash('md5').update(_this.instanceString + 'NodejsDC')
        .digest('hex');
    logger.debug('The plain text of runtime id: ', _this.nodeRuntimeString);
...
JsonSender.prototype.registerAppModelOnPre = function registerAppModelOnPre(reqData) {
    // if (this.vcap || this.isicp || global.RESOURCE_SVCEP_REGISTED) {
    //     return;
    // }
    // global.RESOURCE_SVCEP_REGISTED = true;

    var host = reqData.request.context.requestHeader.host;
...
// if (k8sutil.isICP()) {
//     logger.debug('json-sender.js', 'this is ICp.');
//     // In the ICp, need to Register the k8s resource.
//     var controller = require('./tool/controller');
//     var event = controller.getEvent();
//     event.once('ready_4_res_on_icp', function() {
//         registerK8sResOnICP();
//     });
// }

// function registerK8sResOnICP() {
//     logger.debug('json-sender.js', 'registerK8sResOnICP');
//     let pod = k8sutil.getPodDetail();
//     let podid = '';
//     if (pod) {
//         podid = formatK8sID('pod', pod.metadata.name, pod.metadata.namespace);
//         let podObj = {
//             id: podid,
//             type: ['pod'],
//             properties: {
...
//                 kubeStartedAt: pod.status.startTime
//             }
//         };
//         restClient.registerResource(podObj);
//     }
//     let nodes = k8sutil.getNodes();
//     for (let index = 0; index < nodes.length; index++) {
//         const node = nodes[index];
//         let nodeObj = {
//             id: formatK8sID('node', node.metadata.name),
//             type: ['node'],
//             properties: {
//                 nodeName: node.metadata.name
//             },
//             references: [
//                 { direction: 'to', type: 'hosts', id: podid }
//             ]
//         };
//         restClient.registerResource(nodeObj);
//     }

//     let services = k8sutil.getServicesConn();
//     for (let index = 0; index < services.length; index++) {
//         const service = services[index];
//         let svcObj = {
//             id: formatK8sID('service', service.name, service.namespace),
//             type: ['service'],
//             properties: {
//                 namespace: service.namespace,
//                 clusterIP: service.clusterIP,
//                 mergeTokens: service.mergeTokens,
//                 ports: service.ports
//             },
//             references: [
//                 { direction: 'to', type: 'loadBalances', id: podid }
//             ]
//         };

//         for (let indexsvc = 0; indexsvc < serviceEndPointResIDs.length; indexsvc++) {
//             let svcep = serviceEndPointResIDs[indexsvc];
//             svcObj.references.push({ direction: 'to', type: 'assignedTo', id: svcep });
//         }
//         restClient.registerResource(svcObj);
//     }

//     let container = k8sutil.getContainerDetail();
//     if (container) {
//         let conObj = {
//             id: formatK8sID('container', container.name,
//                 k8sutil.getNamespace(), k8sutil.getPodName()),
//             type: ['container'],
//             properties: {
//                 containerName: container.name,
//                 containerID: container.containerID,
//                 namespace: k8sutil.getNamespace(),
//                 mergeTokens: k8sutil.getNodeName() + '.' + k8sutil.getNamespace() +
//                     '.' + k8sutil.getPodID() + '.' + k8sutil.getContainerName()
//             },
//             references: [
//                 { direction: 'to', type: 'assignedTo', id: instanceResID },
//                 { direction: 'from', type: 'members', id: podid }
//             ]
//         };
//         restClient.registerResource(conObj);
//     }

// };

// function formatK8sID(type, name, namespace, metadata) {
//     logger.debug('json-sender.js', 'formatK8sID', type, name, namespace, metadata);
//     let id = '';
...
JsonSender.prototype.send = function send(data) {
    ...
    if (this.nodeRuntimeMD5String) {
        resid1.runtimeResourceID = this.nodeRuntimeMD5String;
    }
    ...
    if (this.nodeAppMD5String) {
        resid2.applicationResourceID = this.nodeAppMD5String;
    }
    if (this.instanceMD5String) {
        resid1.applicationInstanceResourceID = this.instanceMD5String;
        resid2.applicationInstanceResourceID = this.instanceMD5String;
    }
    var reqSummPayload = this.genReqSumm(commonTools.merge([dimensions_content, resid1]), data);
    metricPayloads.push(reqSummPayload);

    var sysPayload = this.genSysInfo(commonTools.merge([dimensions_content, resid1]), data);
    metricPayloads.push(sysPayload);

    if (this.nodeRuntimeMD5String) {
        var enginePayloadMeta = {
            resourceID: this.nodeRuntimeMD5String,
...
JsonSender.prototype.genELPayload = function genELPayload(dimensions_content,
    data, enginePayloadMeta) {
    var elPayload = commonTools.merge([
        enginePayloadMeta, {
            metrics: {
            ...
            dimensions: commonTools.merge([dimensions_content,
...
JsonSender.prototype.genSysInfo = function genReqSumm(dimensions_content, data) {
