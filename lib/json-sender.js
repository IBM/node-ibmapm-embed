            ...
            restClient.postDCConfiguration({
                ...
                capabilities: {
                    ...
                    enableTransactionTracking:
                        dcObj.properties.transactionTrackingEnabled ? 'true' : 'false',
                    enableDiagnosticData:
                        dcObj.properties.diagnosticsEnabled ? 'true' : 'false',
                    enableMethodTracing:
                        dcObj.properties.methodTraceEnabled ? 'true' : 'false',
                    enableMetrics: 'true'
                }
                            ...
                            restClient.getConfiguration(function(geterr, getres){
                                ...
                                if (getres){
                                    getres.on('data', function(d){
                                        ...
                                        dealwithConfigurationChange(JSON.parse(configuration));
                                    });
                                }
                            });
                        }, 60000);
...
function dealwithConfigurationChange(conf){
    ...
    if (conf.configuration) {
        ...
        if (conf.configuration.capabilities.enableTransactionTracking
            ...
            process.env.KNJ_ENABLE_TT
                = conf.configuration.capabilities.enableTransactionTracking;
            updated = true;
        }
        ...
        if (conf.configuration.properties.SAMPLING_COUNT.value
            ...
            process.env.KNJ_SAMPLING
                = conf.configuration.properties.SAMPLING_COUNT.value + '';
            updated = true;
        }
        if (conf.configuration.properties.MIN_CLOCK_TRACE.value
            !== parseInt(process.env.KNJ_MIN_CLOCK_TRACE)) {
            process.env.KNJ_MIN_CLOCK_TRACE
                = conf.configuration.properties.MIN_CLOCK_TRACE.value + '';
            updated = true;
        }
        if (conf.configuration.properties.MIN_CLOCK_STACK.value
            !== parseInt(process.env.KNJ_MIN_CLOCK_STACK)) {
            process.env.KNJ_MIN_CLOCK_STACK
                = conf.configuration.properties.MIN_CLOCK_STACK.value + '';
            updated = true;
        }
        if (conf.configuration.properties.EVENTS_PER_FILE.value
            !== parseInt(process.env.KNJ_EVENTS_PER_FILE)) {
            process.env.KNJ_EVENTS_PER_FILE
                = conf.configuration.properties.EVENTS_PER_FILE.value + '';
            updated = true;
        }
        if (conf.configuration.properties.FILE_COMMIT_TIME.value
            !== parseInt(process.env.KNJ_FILE_COMMIT_TIME)) {
            process.env.KNJ_FILE_COMMIT_TIME
                = conf.configuration.properties.FILE_COMMIT_TIME.value + '';
            updated = true;
        }
        if (updated){
    ...
    if (process.mainModule.paths && process.mainModule.paths.length > 0) {
        ...
        for (let i = 0; i < process.mainModule.paths.length; i++) {
            var packageFile = process.mainModule.paths[i].split('node_modules')[0] +
                '/' + 'package.json';
            try {
                const packageStr = fs.readFileSync(packageFile);
                const packJson = JSON.parse(packageStr);
                this.appVersion = packJson.version;
                if (packJson.dependencies) {
                    npmObj['VersionDependencies'] = {};
                }
                for (let dkey in packJson.dependencies) {
                    if (packJson.dependencies.hasOwnProperty(dkey)) {
...
JsonSender.prototype.dynamicRegister = function dynamicRegister(env) {
    ...
    var nodeappPayload = {
        ...
        deployment: getDeployment(),
        properties: commonTools.merge([{
            version: this.appVersion ? this.appVersion : '1.0.0',
            startedTime: this.startTime,
            name: this.applicationName,
    ...
    restClient.registerAppResource(nodeappPayload, function(err, res){
        if (err){}
        if (res) {
            if (res.statusCode >= 200 && res.statusCode < 300 || res.statusCode === 409) {
                logger.debug('service is registered');
                global.serviceIsRegistered = true;
            }
        }
    });
...
JsonSender.prototype.send = function send(data) {
    ...
    if (global.serviceIsRegistered && global.nodeApplicationRuntimeIsRegistered
