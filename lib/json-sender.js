    ...
    restClient.registerDC(dcObj, function(error, res) {
        ...
        if (res && ((res.statusCode >= 200 && res.statusCode < 300) || res.statusCode === 409)) {
            queryProviderInterval = setInterval(function(){
                restClient.getDC(function(error,res){
                    queryProviderRetry++;
                    if (error) {
                    ...
                    if (queryProviderRetry >= 10) {
                        clearInterval(queryProviderInterval);
                        logger.error('Failed to get provider 10 seconds after registered, please contact your server admin.');
                    }
            ...
            }, 1000);   // queryProviderInterval end
        }
    });
};
